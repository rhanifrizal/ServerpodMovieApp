...
class UserConfirmRegistrationUseCase implements UseCase<User, UserConfirmRegistrationParams> {
  ...
  Future<Either<Failure, User>> call(UserConfirmRegistrationParams params) async {
    final confirmationResult = await authRepository.confirmRegistration(email: params.email, verificationCode: params.verificationCode);

    return await confirmationResult.fold(
...
class UserConfirmRegistrationParams {
  ...
  const UserConfirmRegistrationParams({required this.email, required this.verificationCode, required this.password});
}