...
class UserRegisterUseCase implements UseCase<bool, UserRegisterParams> {
  ...
  Future<Either<Failure, bool>> call(UserRegisterParams params) async {
    return await authRepository.registerWithEmailAndPassword(email: params.email, password: params.password, username: params.username);
  }
}
...
class UserRegisterParams {
  ...
  const UserRegisterParams({required this.email, required this.password, required this.username});
}