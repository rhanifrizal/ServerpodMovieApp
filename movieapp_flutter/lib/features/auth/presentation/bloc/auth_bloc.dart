...
class AuthBloc extends Bloc<AuthEvent, AuthState> {
  ...
  FutureOr<void> _onAuthRegister(AuthRegisterEvent event, Emitter<AuthState> emit) async {
    final result = await userRegister(UserRegisterParams(email: event.email, password: event.password, username: event.username));
  ...
  FutureOr<void> _onAuthConfirmRegistration(AuthConfirmRegistrationEvent event, Emitter<AuthState> emit) async {
    final result = await userConfirmRegistration(UserConfirmRegistrationParams(email: event.email, password: event.password, verificationCode: event.verificationCode));
