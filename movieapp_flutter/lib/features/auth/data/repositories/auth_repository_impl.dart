...
import 'package:movieapp_flutter/features/auth/domain/repositories/auth_repository.dart';
...
class AuthRepositoryImpl implements AuthRepository {
  ...
  @override
  Future<Either<Failure, bool>> registerWithEmailAndPassword({required String email, required String password, required String username}) async {
    try {
      final success = await dataSource.registerWithEmailAndPassword(email: email, password: password, username: username);
  ...
  @override
  Future<Either<Failure, User>> confirmRegistration({required String email, required String verificationCode}) async {
    try {
      final user = await dataSource.confirmRegistration(email: email, verificationCode: verificationCode); 
      return right(user);
    } on ServerException catch (e) {
      return left(Failure(e.message));
    }
